  ...
  return (
    <section className="relative py-24 md:py-32 overflow-hidden border-y bg-dark-950 border-zinc-800">
      <div className="container mx-auto px-4 max-w-5xl">
        ...
        <h2 className="font-display text-3xl md:text-4xl lg:text-5xl font-bold text-center mb-4 tracking-tight text-zinc-100">
        ...
        <div className={cn(
          'rounded-3xl border shadow-lg p-10 md:p-14 text-center transition-opacity duration-200 bg-dark-800 border-zinc-700',
          animating ? 'opacity-0' : 'opacity-100'
        )}>
          <Quote className="w-10 h-10 mx-auto mb-8 text-violet-400/30" />
          ...
          <blockquote className="text-xl md:text-2xl font-medium leading-relaxed mb-10 max-w-3xl mx-auto text-zinc-200">
            &ldquo;{t.quote}&rdquo;
          </blockquote>
        ...
        <div className="flex items-center justify-center gap-6 mt-10">
          <button onClick={() => goTo((prev) => (prev - 1 + testimonials.length) % testimonials.length)}
            className="w-11 h-11 rounded-full border flex items-center justify-center hover:text-violet-600 hover:border-violet-300 hover:bg-violet-50 transition-all shadow-sm border-zinc-700 bg-dark-800 text-zinc-400" aria-label="Previous">
            <ChevronLeft className="w-5 h-5" />
          </button>
          <div className="flex items-center gap-2">
            {testimonials.map((_, i) => (
              <button key={i} onClick={() => goTo(() => i)}
                className={cn('h-2 rounded-full transition-all duration-400', i === active ? 'w-8 bg-violet-500' : 'w-2 bg-zinc-600 hover:bg-zinc-500')}
                aria-label={`Go to ${i + 1}`} />
            ))}
          </div>
          <button onClick={() => goTo((prev) => (prev + 1) % testimonials.length)}
            className="w-11 h-11 rounded-full border flex items-center justify-center hover:text-violet-600 hover:border-violet-300 hover:bg-violet-50 transition-all shadow-sm border-zinc-700 bg-dark-800 text-zinc-400" aria-label="Next">
